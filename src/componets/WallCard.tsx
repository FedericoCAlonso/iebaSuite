...
export function WallCard({ pared, index, onChange, onRemove }) {
  ...
  const setIrr = (irrs) => onChange({...pared, irregularidades:irrs});
  ...
  return (
    ...
    >
      <div className="field-row">
        ...
        <F label="Ángulo (°)">
          <NumInput value={pared.angulo??90} onChange={v=>onChange({...pared,angulo:v})} placeholder="90"/>
        </F>
      </div>
      <div className="field-row">
        <F label="Grosor propio (m)">
          <NumInput value={pared.grosor??''} onChange={v=>onChange({...pared,grosor:v||null})} placeholder="default" step={0.01}/>
        </F>
        <F label="Mocheta (m, vacío=no)">
          <NumInput
            value={pared.esquina_saliente?.ancho??''}
            onChange={v=>onChange({...pared,esquina_saliente:v?{ancho:v}:null})}
            placeholder="no" step={0.01}
          />
        </F>
      </div>
          ...
          <div className="sub-card-body">
            {(pared.irregularidades||[]).map((irr,j)=>(
              <div key={j} className="irr-row">
                ...
                <F label="Pos(m)"><NumInput value={irr.posicion} onChange={v=>{const a=[...pared.irregularidades];a[j]={...irr,posicion:v};setIrr(a);}} step={0.01} style={{width:58}}/></F>
                <F label="Ancho"><NumInput value={irr.ancho} onChange={v=>{const a=[...pared.irregularidades];a[j]={...irr,ancho:v};setIrr(a);}} step={0.01} style={{width:58}}/></F>
                <F label="Prof(+/-)"><NumInput value={irr.profundidad} onChange={v=>{const a=[...pared.irregularidades];a[j]={...irr,profundidad:v};setIrr(a);}} step={0.01} style={{width:58}}/></F>
